...
func (s *Store) Comparison(ctx context.Context, query regression.Query) (regression.Input, error) {
	return comparison(ctx, s.db, query)
}

func (s *ReadSnapshot) Comparison(ctx context.Context, query regression.Query) (regression.Input, error) {
	return comparison(ctx, s.tx, query)
}

func comparison(ctx context.Context, db queryer, query regression.Query) (regression.Input, error) {
	...
	var started string
	err := db.QueryRowContext(ctx, `SELECT id,external_id,environment,service_key,started_at FROM deployments WHERE id=?`, query.DeploymentID).Scan(&input.Deployment.ID, &input.Deployment.ExternalID, &input.Deployment.Environment, &input.Deployment.Service, &started)
	...
	err = db.QueryRowContext(ctx, `SELECT id FROM services WHERE environment=? AND logical_key=?`, input.Deployment.Environment, input.Deployment.Service).Scan(&input.Target.ID)
	...
	input.Before, err = comparisonWindows(ctx, db, input.Deployment.Environment, input.Deployment.Service, beforeStart, input.Deployment.StartedAt)
	if err != nil {
		return regression.Input{}, err
	}
	input.After, err = comparisonWindows(ctx, db, input.Deployment.Environment, input.Deployment.Service, input.Deployment.StartedAt, afterEnd)
	if err != nil {
		return regression.Input{}, err
	}
	input.Contamination, err = comparisonContamination(ctx, db, input.Deployment, beforeStart, afterEnd)
	if err != nil {
		return regression.Input{}, err
	}
	return input, nil
...
func comparisonContamination(ctx context.Context, db queryer, selected regression.Deployment, beforeStart, afterEnd time.Time) (regression.Contamination, error) {
	result := regression.Contamination{BeforeDeployments: []string{}, AfterDeployments: []string{}, ConcurrentDeployments: []string{}}
	rows, err := db.QueryContext(ctx, `SELECT id,started_at FROM deployments WHERE environment=? AND service_key=? AND id<>? AND started_at>=? AND started_at<? ORDER BY started_at,id LIMIT ?`, selected.Environment, selected.Service, selected.ID, formatTime(beforeStart), formatTime(afterEnd), regression.MaxContaminatingDeployments+1)
	if err != nil {
...
var _ regression.Reader = (*ReadSnapshot)(nil)
